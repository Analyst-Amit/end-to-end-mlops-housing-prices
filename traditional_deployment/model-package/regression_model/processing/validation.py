...
def drop_na_inputs(*, input_data: pd.DataFrame) -> pd.DataFrame:
    ...
    new_vars_with_na = [
        var
        for var in config.mod_config.features
        if var
        not in config.mod_config.categorical_vars_with_na_frequent
        + config.mod_config.categorical_vars_with_na_missing
        + config.mod_config.numerical_vars_with_na
...
def validate_inputs(*, input_data: pd.DataFrame) -> Tuple[pd.DataFrame, Optional[dict]]:
    ...
    input_data.rename(columns=config.mod_config.variables_to_rename, inplace=True)
    input_data["MSSubClass"] = input_data["MSSubClass"].astype("O")
    relevant_data = input_data[config.mod_config.features].copy()
    ...
    try:
        ...
        MultipleHouseDataInputs(
            ...
            inputs=validated_data.where(pd.notna(validated_data), None).to_dict(
                orient="records"
            )
        )
...
class HouseDataInputSchema(BaseModel):
    ...
    ThreeSsnPortch: Optional[int] = None  # renamed
